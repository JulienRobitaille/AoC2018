import strutils, strscans, sequtils, system, algorithm, tables
...
[1518-11-10 00:33] falls asleep""".splitLines

const sortedInput = sorted(input, cmp)
var guardsSleepTime = initTable[int, int]()
var guardsSleepMinute = initTable[int, seq[int]]()
var id = 0;
var asleepAt = 0
for entry in sortedInput:
    var 
        data = entry.splitWhitespace
        minute = 0
        hour = 0
    if scanf(data[1], "$i:$i]", hour, minute):
        if data[2] == "Guard":
            id = data[3][1..data[3].len-1].parseInt
            discard guardsSleepTime.hasKeyOrPut(id, 0)
            discard guardsSleepMinute.hasKeyOrPut(id, @[])
        if data[2] == "falls":
            asleepAt = minute
            guardsSleepMinute[id].add(minute)
        if data[2] == "wakes":
            var difference = minute - asleepAt;
            guardsSleepTime[id] = guardsSleepTime[id] + difference
            for i in 1..difference - 1:
                guardsSleepMinute[id].add(minute - i)


var biggest = @[0, 0, 0] # -- Answer part one
var biggest2 = @[0, 0, 0] # -- Answer part two
for guardId, totalSleep in guardsSleepTime.pairs:

    var guardMin = initTable[int, int]()
    var biggestResult = 0
    var biggestMinute = 0
    
    for minute in guardsSleepMinute[guardId]:
        discard guardMin.hasKeyOrPut(minute, 0)
        guardMin[minute] = guardMin[minute] + 1
    
    
    for minute, result in guardMin:
        if biggestResult < result:
            biggestMinute = minute
            biggestResult = result
    
    if biggest[1] < totalSleep:
        biggest = @[guardId, totalSleep, biggestMinute]        
    if biggest2[1] < biggestResult:
        biggest2 = @[guardId, biggestResult, biggestMinute]        

echo biggest, " - ", biggest[0] * biggest[2]
echo biggest2, " - ", biggest2[0] * biggest2[2]